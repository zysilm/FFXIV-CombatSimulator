...
状态:**已实现，但本文是历史设计记录**。当前行为以 `Camera/DynamicCameraSolver.cs`、
`Camera/DynamicCameraController.cs` 和 `Gui/MainWindow.DynamicCamera.cs` 为准。

截至 2026-07-16 的文档审计:

- 战斗态 framing、自动左右肩位、拥挤拉远、zoom/pitch 记忆已实现。
- 死亡态 grounded fit、镜头松弛阶梯、尸体 coverage、忽略凶手、translate 和诊断 overlay 已实现。
- §6.5 所述“死亡态俯仰读回学习”已被实测否决；当前俯仰由滑条开环控制，垂直输入被锁定。
- “软偏航辅助”已明确删除，不是待办；玩家始终拥有 yaw。
- 连续 FoV breathing 未实现；当前使用基础镜头与加宽镜头的离散松弛阶梯。
- 下方 v1 §6–§10 仅保留用于解释失败模型，不应作为当前实现说明。
...
1. **P1 已完成**:owner 枚举 + CameraRequest.Fov + 凶手记录 + 控制器骨架 + 调试 overlay。
2. **P2 已完成**:侧偏 pivot + 占比距离 + 拥挤拉远 + 弹簧/迟滞。
3. **P3 已完成并重构**:旧双锚线性解已由 grounded fit 取代；松弛阶梯和 translate 保留。
4. **P4 部分完成/部分否决**:战斗态 zoom/pitch 记忆已完成；死亡态俯仰读回、软偏航辅助被
   实测否决；连续 FoV breathing 未实现，当前用离散镜头阶梯。
